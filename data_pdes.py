...
def get_mgrid(sidelen, vmin=-1, vmax=1, dim=2):
    ...
    if isinstance(sidelen, int):
        ...
        if isinstance(vmin, (list, tuple)) and isinstance(vmax, (list, tuple)):
            tensors = tuple(
                [
                    torch.linspace(mi, ma, steps=l)
                    for mi, ma, l in zip(vmin, vmax, sidelen)
                ]
            )
        else:
...
class WaveDataset(AbstractDataset):
    def __init__(self, param, coords="ang", *args, **kwargs):
        ...
        self.grid = CartesianGrid([[-1.0, 1.0]] * 2, self.size, periodic=True)
    ...
    def _get_init_cond(self, index):
        ...
        init_cond = np.roll(
            init_cond, np.random.randint(self.size, size=2), axis=(0, 1)
        )
    ...
    def _generate_trajectory(self, traj_id):
        ...
        self.eqs.solve(
            state,
            t_range=self.t_horizon * self.n_seq_per_traj,
            dt=1e-3,
...
class NavierStokesDataset(AbstractDataset):
    ...
    def navier_stokes_2d(self, w0, f, visc, T, delta_t, record_steps):
        ...
        sol = torch.zeros(
            *w0.size(), record_steps, 1, device=w0.device, dtype=torch.float
        )
    ...
    def _get_init_cond(self, index, start, end):
        ...
        if self.buffer.get(f"init_cond_{index}") is None:
            ...
            state, _ = self.navier_stokes_2d(
                w0,
                f=self.params_eq["f"].to(self.device),
            ...
            init_cond = torch.from_numpy(
                torch.stack(self.buffer[f"init_cond_{i}"] for i in range(start, end))
            )
    ...
    def _generate_trajectory(self, traj_id):
        ...
        with torch.no_grad():
            ...
            state, _ = self.navier_stokes_2d(
                ...
                f=self.params_eq["f"].to(self.device),
                visc=self.params_eq["visc"],
                T=self.t_horizon * self.n_seq_per_traj,
                delta_t=self.dt,
...
class ShallowWaterDataset(AbstractDataset):
    def __init__(self, root, *args, **kwargs):
        ...
        self.dataset_path = os.path.join(
            root, f"shallow_water_{'test' if self.group == 'test' else 'train'}"
        )
        ...
        if self.group == "test_hr":
            phi = torch.tensor(
                self.files_obj_buf[0]["tasks/vorticity"].dims[1][0][:].ravel()
            )
            theta = torch.tensor(
                self.files_obj_buf[0]["tasks/vorticity"].dims[2][0][:].ravel()
            )
        else:
            phi = torch.tensor(
                self.files_obj_buf[0]["tasks/vorticity"].dims[1][0][:].ravel()[::2]
            )
            theta = torch.tensor(
                self.files_obj_buf[0]["tasks/vorticity"].dims[2][0][:].ravel()[::2]
            )
    ...
    def _load_trajectory(self, traj_id, file_object_only=False):
        ...
        if self.group == "test_hr":
            return {
                "data": torch.stack(
                    [
                        torch.from_numpy(f["tasks/height"][...]) * 3000.0,
        ...
        return {
            "data": torch.stack(
                [
                    torch.from_numpy(f["tasks/height"][:, ::2, ::2]) * 3000.0,
...
class SST(Dataset):
    ...
    def __init__(self, data_dir, nt_cond, nt_pred, train, zones=range(1, 30)):
        ...
        self.coords = get_mgrid(self.zone_size, vmin=-1.0, vmax=1.0, dim=2)
        self.coord_dim = self.coords.shape[-1]
    ...
    def _normalize(self):
        for zone in self.zones:
            zdata = extract_data(
                os.path.join(self.data_dir, f"data_{zone}.nc"), variables=self.var_names
            )
            self.len_ = len(zdata["thetao"])

            climate_mean, climiate_std = (
                zdata["daily_mean"].reshape(-1, 1, 1),
                zdata["daily_std"].reshape(-1, 1, 1),
            )
    ...
    def __getitem__(self, idx):
        ...
        inputs = self.data[file_id][idx_id - self.nt_cond + 1 : idx_id + 1].reshape(
            self.nt_cond, 1, self.zone_size, self.zone_size
        )
        target = self.data[file_id][idx_id + 1 : idx_id + self.nt_pred + 1].reshape(
            self.nt_pred, 1, self.zone_size, self.zone_size
        )
        ...
        return {
            ...
            "std_clim": self.climato[file_id][1][
                idx_id + 1 : idx_id + self.nt_pred + 1
            ],
