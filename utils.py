...
def process_config(
    input_dataset, path_results, device="gpu:0", mask_data=0.0, n_frames_train=10
):
...
def generate_mask(xy_size, device, mask_data=0, n_mask=1):
    mask_list = []
    for _ in range(n_mask):
        mask_list.append((torch.rand(xy_size) >= mask_data)[None, :])
...
def write_image(
    coords, batch_gt, batch_pred, state_idx, path, cmap="viridis", divider=1
):
    """
