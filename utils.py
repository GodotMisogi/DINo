...
def process_config(
    ...
    if input_dataset == "wave":
        ...
        buffer_shelve_tr = buffer_shelve_tr_eval = shelve.open(buffer_file_tr)
        buffer_shelve_ts = shelve.open(buffer_file_ts)
        dataset_tr = NavierStokesDataset(
            buffer_shelve=buffer_shelve_tr, **dataset_tr_params
        )
        dataset_tr_eval = NavierStokesDataset(
            buffer_shelve=buffer_shelve_tr_eval, **dataset_tr_eval_params
        )
        dataset_ts = NavierStokesDataset(
            buffer_shelve=buffer_shelve_ts, **dataset_ts_params
        )
        ...
        dataset_ts = SplashDataset(**dataset_ts_params)
    else:
    ...
    if input_dataset == "shallow_water_hs":
        ...
        mask_ts = generate_skipped_lat_lon_mask(
            dataset_ts.coords_ang, device, base_jump=1
        ).bool()
        ...
        if lr_adapt != 0.0:
            ...
            for i in range(n_steps):
                ...
                model_input_exp = model_input_exp.expand(
                    b_size, 1, h_size, w_size, state_dim, coord_dim
                )
        ...
        with torch.no_grad():
            ...
            if mask_data != 0.0:
                loss_in_t_in_s += criterion(
                    model_output[:, :n_frames_train, mask, :],
                ...
                loss_out_t_in_s += criterion(
                    model_output[:, n_frames_train:, mask, :],
                    ground_truth[:, n_frames_train:, mask, :],
                )
                loss_out_t_out_s += criterion(
                    model_output[:, n_frames_train:, ~mask, :],
                    ground_truth[:, n_frames_train:, ~mask, :],
                )
    ...
    for j, batch in enumerate(dataloader):
        ...
        if lr_adapt != 0.0:
            ...
            for i in range(n_steps):
                ...
                model_input_exp = model_input_exp.expand(
                    b_size, n_cond + 1, h_size, w_size, state_dim, coord_dim
                )
                model_output, _ = net_dec(model_input_exp, states)
                model_output = revin(model_output, mode="denorm")
                loss_l2 = criterion(
        ...
        with torch.no_grad():
            ...
            model_input_exp = model_input.view(b_size, 1, h_size, w_size, 1, coord_dim)
            model_input_exp = model_input_exp.expand(
                b_size, t_size - n_cond, h_size, w_size, state_dim, coord_dim
            )
...
def write_image(batch_gt, batch_pred, state_idx, path, cmap="RdBu_r", divider=1):
    ...
    for traj in range(batch_size):
        gt = batch_gt[:, traj, :, :, state_idx].cpu().numpy()
        pr = batch_pred[:, traj, :, :, state_idx].cpu().numpy()
        vmax = np.max(gt)
        vmin = np.min(gt)
...
#     seq_len, batch_size, height, width, state_c = batch_gt.shape  # [8, 20, 64, 64, 2]
#     t_horizon = math.ceil(seq_len / divider)
#     fig = plt.figure(figsize=(t_horizon, batch_size * 3))
#     grid = ImageGrid(
#         fig,
#         111,  # similar to subplot(111)
#         nrows_ncols=(batch_size * 3, t_horizon),  # creates 2x2 grid of axes
...
#                 interpolation="none",
#             )
#             grid[(2 * traj + 2) * t_horizon + t].imshow(
#                 np.abs(gt - pr),
#                 vmax=vmax,
#                 vmin=vmin,
#                 cmap=cmap,
...
#             grid[(2 * traj + 2) * t_horizon + t].set_axis_off()
