...
for epoch in range(n_epochs):
    if n_cond == 0:
        ...
        for i, batch in enumerate(dataloader_tr):
            ...
            if epoch == 0 and i == 0:
                ...
                # plt.savefig(
                #     os.path.join(path_checkpoint, f"mask_ts.png"),
                #     dpi=72,
                #     bbox_inches="tight",
                #     pad_inches=0,
                # )
            ...
            if (epoch * len(dataloader_tr) + i) % log_every == 0:
                logger.info(
                    "Dataset %s, Runid %s, Epoch [%d/%d] MSE Auto-dec %0.6e, MSE Dyn %0.6e, epsilon %0.6e"
    ...
    #             # logger.info(
    #             #     f"Missingness: {100.0 * (1 - torch.sum(mask) / xy_size)}%"
    #             # )
    #             logger.info(f"ground_truth: {list(ground_truth.size())}")
    #             logger.info(f"t: {t[0]}")
    #             logger.info(f"index: {index}")
    ...
    #             extra_states.append(
    #                 net_cond(
    #                     states_params_index[jjj : jjj + n_cond]
    #                     .permute(1, 0, 2)
    #                     .detach()
    #                     .clone()
    #                 )
    ...
    #             [extra_states, states_params_index[n_cond:].detach().clone()], dim=-1
    #         )

    #         codes = scheduling(
    #             odeint, net_dyn, augmented_states, t, epsilon_t, method=method
    #         )
    #         loss_l2_states = criterion(
    #             codes[:, :, code_dim * state_dim :],
    #             states_params_index[n_cond:].detach().clone(),
    #         )
    ...
    #         loss_l2_ = criterion(
    #             # model_output_[:, :, mask, :], ground_truth_[:, :, mask, :]
    #             model_output, ground_truth
    #         )
    ...
    #             print("Evaluating train...")
    #             loss_tr, loss_tr_in_t, gts, mos, times, ss, pss, cs = eval_dino_cond(
    #                 dataloader_tr,
    #                 net_dyn,
    #                 net_dec,
    #                 net_cond,
    ...
    #                     f"Checkpoint created: min tr loss was {loss_tr_min}, new is {optimize_tr}"
    #                 )
    #                 # for j, (ground_truth, model_output, codes, states, t) in enumerate(
    #                 #     zip(gts, mos, cs, ss, times)
    #                 # ):
    #                 #     if j in [0]:
    #                 #         for state_idx in range(state_dim):
    #                 #             write_image(
    ...
    #                 #                 state_idx,
    #                 #                 os.path.join(
    #                 #                     path_checkpoint, f"img_tr_state{state_idx}.pdf"
    #                 #                 ),
    #                 #             )
    #                 loss_tr_min = optimize_tr
    #                 torch.save(
    ...
    #             print("Evaluating test...")
    #             loss_ts, loss_ts_in_t, gts, mos, times, ss, pss, cs = eval_dino_cond(
    #                 dataloader_ts,
    #                 net_dyn,
    #                 net_dec,
    #                 net_cond,
    ...
    #             logger.info(
    #                 "Dataset %s, Runid %s, Epoch %d, Iter %d, Loss_ts: %.3e In-t: %.3e"
    #                 % (input_dataset, ts, epoch + 1, i + 1, loss_ts, loss_ts_in_t)
    #             )
    #             logger.info("========")
